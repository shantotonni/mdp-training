...
export default {
  ...
  mounted() {
    document.title = 'Job Description Edit | Job Description';
    this.getEditData();
  },
  created() {
    //
  ...
  methods: {
    getEditData(){
      axios.get(baseurl + `api/job-description-edit/${this.$route.params.ID}`).then((response)=>{
        this.form.fill(response.data.job_description);
        this.getAllDivision();
    ...
    update(){
      this.form.busy = true;
      this.form.post(baseurl + "api/job-description-update").then(response => {
