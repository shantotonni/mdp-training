                ...
                <div class="row">
                  ...
                  <div class="col-8">
                    <report-pagination
                        ...
                        :offset="5"
                        @paginate="form.Query === '' ? PendingPTCReport() : PendingPTCReport()"
                    ></report-pagination>
                  </div>
                </div>
...
export default {
  ...
  mounted() {
    document.title = 'Pending PTC | MDP'
    this.PendingPTCReport();
  },
  created() {
    //
  },
  methods: {
    PendingPTCReport(){
      this.isLoading = true;
      this.form.post(baseurl + 'api/mdp-pending-ptc-report').then((response)=>{
    ...
    exportReport(){
      ...
      this.form.post(baseurl + "api/mdp-pending-ptc-report").then(response => {
        ...
        if (dataSets.length > 0) {
          ...
          bus.$emit('data-table-import', dataSets, columns, 'Pending PCT Report Export')
        }
      }).catch(e => {
        //
      });
