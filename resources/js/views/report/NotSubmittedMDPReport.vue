<template>
  <div class="content">
    <div class="container-fluid">
      <breadcrumb :options="['Not Submitted MDP List Report']">
        <div class="col-md-6">
          <div class="float-right d-none d-md-block">
            <div class="card-tools">
              <button class="btn btn-outline-dark btn-sm" v-if="type ==='admin'">
                <i class="fas fa-address-card"></i>
                Not Submitted MDP Count- {{this.mdpNo}}
              </button>
...
<!--                            <option v-for="(session,index) in sessions" :value="session.Name" :key="index">{{session.Name}}</option>-->
                ...
                <div class="table-responsive">
                  ...
                  <pagination
                      ...
                      :offset="5"
                      @paginate="getAllMDPList()"></pagination>
                </div>
              </div>
            </div>
            <div v-else>
...
export default {
  ...
  mounted() {
    document.title = 'Not Submitted MDP List Report | MDP';
    this.getAllMDPList();
    this.getAllSession();
    this.getAllBusiness();
  ...
  methods: {
    getAllMDPList(val){
      ...
      this.PreLoader = true;
      axios.get(baseurl + 'api/mdp/not-submitted-list?page='+ this.pagination.current_page
        ...
        this.getAllEmpID()
        this.mdplist = response.data.data.data;
        // this.pagination = response.data.data.meta;
        this.mdpNo = response.data.data.total
    ...
    exportMDPList(){
      if(this.sessionP.length>0){
        ...
        axios.get(baseurl + 'api/mdp/not-submitted-list?page='+ this.pagination.current_page
          ...
          let dataSets = response.data.exportdata;
